...
+ MCTS ::
  The */runtime per loop/* is $O(BD) = O(\log N)$. $N$: number of nodes

+ Bilevel MCTS ::
  The */amortized/* runtime per loop over $D$ expansions is

  + $O(\log \log N)$ if _priority queue $Q$ is a heap_

  + $O(1)$ if $Q$ is an _array-based priority queue_ (Dial 1969, Burns 2012)
...
* *Depth Budget* $b=d$ vs. *Fixed Budget* $b=10^k$
...
|        |  <c>   | <c>  |  <c>   |    <c>    |   <c>   |  <c>  |
| /      |   <>   |  <   |        |           |         |   >   |
|        |  b=d   | b=1  |   10   |   10^2    |  10^3   | 10^4  |
|--------+--------+------+--------+-----------+---------+-------|
| cea/18 |  53.8  | 50.2 | *57.6* |   51.8    |   49    | 46.6  |
| cea/23 |  30.8  | 25.8 |  32.4  |  *36.8*   |  33.8   | 33.6  |
| cg/18  | *56.2* | 42.4 |  51.6  |   55.8    |  48.6   | 48.8  |
| cg/23  |  41.6  |  25  |  34.2  |   42.2    |  *44*   | 42.2  |
| ff/18  | *87.4* | 79.8 |  85.8  |   83.4    |  82.2   | 72.8  |
| ff/23  |  58.2  | 23.8 |  45.4  |   56.8    |  *60*   | 58.6  |
|--------+--------+------+--------+-----------+---------+-------|
| total  | *328*  | 247  | ↗307  | ↗326.8↘ | 317.6↘ | 302.6 |

+ *The best budget is different in every setting*
  + _/Too many / too few Best-First Search/_
+ */Dynamic : Handles various scenarios well/*
...
* Additional improvement: Tree Collapsing

If a large $d$ slows down MCTS, _/what if we reduce $d$?/_
...
* N$\epsilon$bula = Bilevel MCTS + BFWS + LAMA

N$\epsilon$bula : 3 best practices in a single planner
+ *Codebase* : Fast Downward
+ *Base search algorithm* : Bilevel MCTS + dynamic tree collapsing
...
+ N$\epsilon$bula: SotA by combining MCTS+LAMA+BFWS
